...
def back_project_granules(
    granule_orbit_pairs: Iterable, work_dir: Path, gpu: bool = False
) -> None:
    ...
    cmd = (
        "sentinel/sentinel_scene_multigpu.py"
        if gpu
        else "sentinel/sentinel_scene_cpu.py"
    )
...
def back_project(
    ...
    for granule in granules:
        granule_path, granule_bbox = utils.download_raw_granule(
            granule, work_dir, unzip=True
        )
    ...
    print(
        f'Finished back-projection for {list(work_dir.glob("S1*.geo"))[0].with_suffix("").name}!'
    )
...
def main():
    ...
    parser.add_argument(
        "--earthdata-username", default=None, help="Username for NASA's EarthData"
    )
    parser.add_argument(
        "--earthdata-password", default=None, help="Password for NASA's EarthData"
    )
    parser.add_argument(
